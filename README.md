...
İstanbul'un 39 ilçesini **kira, hızlı ulaşım ve işe gidiş** açısından birlikte karşılaştıran bir karar aracı.

Ekrandaki her sayının kaynağı ve tarihi yazılıdır. Kaynağı olmayan hiçbir değer gösterilmez - tahmin edilmez, sıfır sayılmaz, "makul bir şey" yazılmaz.

![PropertyPulse ekran görüntüsü](docs/ekran-goruntusu.png)

## Hangi soruyu cevaplıyor

Kira haritası her emlak sitesinde var. Bu araç farklı bir soruya bakıyor: **ucuz semt gerçekten ucuz mu?**

Gelirini, aradığın daire büyüklüğünü ve iş yerini girdiğinde ilçeler hem kiraya hem işe yakınlığa göre değerlendiriliyor. Beşiktaş'ta çalışan biri için sonuç:

| İlçe | Kira (90 m²) | İşe |
|---|---|---|
| Beşiktaş | 45.360 ₺ | 0 durak |
| Şişli | 42.030 ₺ | 3 durak |
| Üsküdar | 36.810 ₺ | 4 durak |
| Kağıthane | 32.220 ₺ | 5 durak |
| Çatalca | 18.000 ₺ | 12 durak + istasyona 32,5 km |

Bu beşi **baskılanmamış** seçenekler: hiçbiri için "hem daha ucuz hem işe daha yakın" bir alternatif yok. Beyoğlu (35.460 ₺, 7 durak) listede yok, çünkü Kağıthane hem daha ucuz hem daha yakın.

Kirayı ve yakınlığı tek puana indirmiyoruz: *"bir durak kaç lira eder"* kişiden kişiye değişir, bizim bilebileceğimiz bir şey değil.

## Öne çıkanlar

- **39 ilçe**, gerçek sınır çokgenleriyle boyanmış harita
- **Hızlı ulaşım**: 314 istasyon (metro, tramvay, Marmaray, füniküler, teleferik, metrobüs); hat kodları ve ilçe içi istasyon listeleriyle
- **İşe gidiş güzergâhı**: 314 düğümlük ağda Dijkstra - kaç durak, kaç aktarma, hangi hatlar
- **Otobüs sefer sıklığı**: durak sayısı değil sıklık (Fatih'te durak başına günde ~455 sefer, Şile'de ~9)
- **Kullanıcı katkısı**: açık kaynağı olmayan veriyi toplama mekanizması
- **Eksik veri gizlenmiyor**: "veri yok" yazıyor, eksik kalem varsa oran işaretleniyor, hiçbir ilçe için "bütçene uygun" hükmü verilmiyor
...
npm run db:deploy             # şemayı uygula (migration'ları çalıştırır)
npm run db:seed               # data/ altındaki veriyi yükle
npm run dev                   # http://localhost:3000
npm test                      # 67 birim test
```

TCMB kira endeksini kendin güncellemek istersen `.env` dosyasına `EVDS_API_KEY` ekle ([evds3.tcmb.gov.tr](https://evds3.tcmb.gov.tr) profil sayfasından ücretsiz alınır). Uygulamanın çalışması için gerekli değil - veri `data/` altında hazır.
...
- **Sıralama nasıl doğrulandı?** Kadıköy → Levent için M4 (Göztepe→Ünalan) → Metrobüs (Uzunçayır→Zincirlikuyu) → M2 (Gayrettepe→Levent) çıkıyor: Ünalan metro ile Uzunçayır metrobüs arasındaki aktarma gerçek ve bu güzergâh İstanbul'da yaygın kullanılıyor. Metrobüs ağa eklenmeden önce aynı yolculuk M4 → Marmaray → M2 üzerinden 12 durak çıkıyordu; metrobüsle 7 durağa indi. Üsküdar → Levent, Esenler → Levent ve Bağcılar → Kabataş da gerçek güzergâhlarla uyuşuyor. Bu kontroller birim testlere yazıldı.
...
- **Veritabanı:** PostgreSQL + Prisma 6, versiyonlu migration
- **Test:** Node'un yerleşik test koşucusu (67 test), ek bağımlılık yok
- **Veri hazırlığı:** Python script'leri (`scripts/`)
- **Tipografi:** Inter (gövde ve rakamlar) + Space Grotesk (başlıklar)
